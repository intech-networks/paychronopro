...
WITH active_employees AS (
  ...
  SELECT employee.*,
         DATE_TRUNC('week', CURRENT_DATE)::date + day_series.day_offset AS work_date
  FROM active_employees employee
  CROSS JOIN GENERATE_SERIES(0,6) AS day_series(day_offset)
  WHERE LOWER(TO_CHAR(DATE_TRUNC('week', CURRENT_DATE)::date + day_series.day_offset,'FMDay')) = ANY(employee.work_days)
), punches AS (
