...
WITH active_employees AS (
  ...
  SELECT work_day.employee_number, work_day.work_date, punch.punch_time
  FROM work_dates work_day
  CROSS JOIN LATERAL (VALUES
    (CASE work_day.exception_type WHEN 'late_first_in' THEN work_day.shift_start + INTERVAL '18 minutes' WHEN 'absent' THEN NULL ELSE work_day.shift_start END),
    (CASE WHEN work_day.exception_type = 'absent' THEN NULL ELSE TIME '12:00' END),
    (CASE work_day.exception_type WHEN 'late_break' THEN TIME '13:20' WHEN 'undertime_short_break' THEN TIME '12:30' WHEN 'late_first_in' THEN TIME '13:00' ELSE NULL END),
    (CASE work_day.exception_type WHEN 'late_break' THEN work_day.shift_end WHEN 'late_first_in' THEN work_day.shift_end WHEN 'undertime_short_break' THEN work_day.shift_end - INTERVAL '60 minutes' ELSE NULL END)
  ) punch(punch_time)
  ...
  ) ORDER BY work_date, employee_number, punch_time), '[]'::jsonb) AS records
  FROM punches
), device_users AS (
