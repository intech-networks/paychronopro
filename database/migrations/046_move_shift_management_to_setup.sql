...
SELECT child_permission.role_id, setup.id,
       child_permission.can_create, child_permission.can_view,
       child_permission.can_update, child_permission.can_delete
FROM role_permissions child_permission
JOIN modules child ON child.id = child_permission.module_id AND child.module_key = 'shift_management'
