...
const taxFrequencyLabels={daily:'Daily',weekly:'Weekly',semi_monthly:'Semi-monthly',monthly:'Monthly'};
function taxConfigurationForm(item){return {...item,exemptionAmount:item.exemptionAmount??'0',effectiveFrom:item.effectiveFrom?.slice(0,10)||'',effectiveTo:item.effectiveTo?.slice(0,10)||'',brackets:item.brackets.map(b=>({...b,upperBound:b.upperBound??''}))};}
function taxAmount(value){return Number(value||0).toLocaleString('en-PH',{minimumFractionDigits:Number(value)%1?2:0,maximumFractionDigits:2});}
function TaxConfiguration({user}){
  ...
  async function load(selectedId='first'){try{const r=await fetch('/api/tax-configurations'),d=await r.json();if(!r.ok)throw new Error(d.error);setItems(d.configurations);const selected=selectedId==='first'?d.configurations[0]:d.configurations.find(item=>String(item.id)===String(selectedId));if(selected)setForm(taxConfigurationForm(selected));}catch(e){setMessage(e.message);}}
  useEffect(()=>{load();},[]);
  function edit(item){setForm(taxConfigurationForm(item));setPreview(null);setMessage('');}
  function bracket(index,key,value){setForm(current=>({...current,brackets:current.brackets.map((b,i)=>i===index?{...b,[key]:value}:b)}));}
  async function save(e){e.preventDefault();setSaving(true);setMessage('');try{const r=await fetch(`/api/tax-configurations${form.id?`/${form.id}`:''}`,{method:form.id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)}),d=await r.json();if(!r.ok)throw new Error(d.error);setMessage(d.message);await load(d.id);}catch(error){setMessage(error.message);}finally{setSaving(false);}}
  ...
  return <section className="tax-config-view">
    <div className="module-title"><div><span>Setup</span><h1>Tax Configuration</h1><p>Maintain the revised withholding tax table by payroll frequency.</p></div><button type="button" onClick={()=>{setForm({...emptyTaxConfiguration});setPreview(null);setMessage('');}}>New configuration</button></div>
    {message&&<p className="rbac-message">{message}</p>}
    <div className="tax-config-layout">
      <aside><div className="tax-list-heading"><strong>Withholding tables</strong><small>Select a schedule to review or edit</small></div>{items.map(item=><button type="button" className={form.id===item.id?'selected':''} key={item.id} onClick={()=>edit(item)}><strong>{taxFrequencyLabels[item.payFrequency]||item.payFrequency}</strong><small>{item.name}<br/>Effective {item.effectiveFrom?.slice(0,10)}</small><b>{item.isActive?'Active':'Inactive'}</b></button>)}{!items.length&&<p>No configurations yet.</p>}</aside>
      <form onSubmit={save}>
        <div className="tax-sheet-title"><strong>REVISED WITHHOLDING TAX TABLE</strong><span>{form.effectiveFrom?`Effective ${new Date(`${form.effectiveFrom}T00:00:00`).toLocaleDateString('en-PH',{year:'numeric',month:'long',day:'numeric'})} onwards`:'Set the effective date below'}</span></div>
        <div className="tax-general"><label><span>Configuration name</span><input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="BIR 2023 onwards — Monthly"/></label><label><span>Pay frequency</span><select value={form.payFrequency} onChange={e=>setForm({...form,payFrequency:e.target.value})}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="semi_monthly">Semi-monthly</option><option value="monthly">Monthly</option></select></label><label><span>Effective from</span><input type="date" required value={form.effectiveFrom} onChange={e=>setForm({...form,effectiveFrom:e.target.value})}/></label><label><span>Effective to</span><input type="date" value={form.effectiveTo} onChange={e=>setForm({...form,effectiveTo:e.target.value})}/></label><label><span>Exemption per pay period (PHP)</span><input type="number" min="0" step=".01" value={form.exemptionAmount} onChange={e=>setForm({...form,exemptionAmount:e.target.value})}/></label><label className="tax-active"><input type="checkbox" checked={form.isActive} onChange={e=>setForm({...form,isActive:e.target.checked})}/>Active for payroll</label></div>
        <div className="tax-table-scroll"><div className="tax-worksheet" style={{'--tax-columns':form.brackets.length}}>
          <div className="tax-frequency-row">{taxFrequencyLabels[form.payFrequency]||form.payFrequency}</div>
          <div className="tax-row-label tax-number-label">Bracket</div>{form.brackets.map((_,i)=><div className="tax-bracket-number" key={`number-${i}`}>{i+1}</div>)}
          <div className="tax-row-label">Compensation Range</div>{form.brackets.map((b,i)=><div className="tax-range-cell" key={`range-${i}`}><label><span>From</span><span className="tax-money-input">₱<input aria-label={`Bracket ${i+1} compensation from`} type="number" min="0" step=".01" required value={b.lowerBound} onChange={e=>bracket(i,'lowerBound',e.target.value)}/></span></label><label><span>{i===form.brackets.length-1?'Upper limit':'To'}</span><span className="tax-money-input">{i===form.brackets.length-1?'':'₱'}<input aria-label={`Bracket ${i+1} compensation upper limit`} type="number" min="0" step=".01" value={b.upperBound} placeholder="No limit" onChange={e=>bracket(i,'upperBound',e.target.value)}/></span></label></div>)}
          <div className="tax-row-label tax-final-row">Prescribed Withholding Tax</div>{form.brackets.map((b,i)=><div className="tax-formula-cell tax-final-row" key={`formula-${i}`}><div className="tax-formula-inputs"><label><span>Base tax</span><span className="tax-money-input">₱<input aria-label={`Bracket ${i+1} base tax`} type="number" min="0" step=".01" required value={b.baseTax} onChange={e=>bracket(i,'baseTax',e.target.value)}/></span></label><label><span>Rate</span><span className="tax-rate-input"><input aria-label={`Bracket ${i+1} rate on excess`} type="number" min="0" max="100" step=".001" required value={b.ratePercent} onChange={e=>bracket(i,'ratePercent',e.target.value)}/>%</span></label></div><small>{Number(b.baseTax)===0&&Number(b.ratePercent)===0?'0':`₱${taxAmount(b.baseTax)} + ${taxAmount(b.ratePercent)}% over ₱${taxAmount(b.lowerBound)}`}</small><button type="button" aria-label={`Remove bracket ${i+1}`} onClick={()=>setForm({...form,brackets:form.brackets.filter((_,index)=>index!==i)})}>Remove</button></div>)}
        </div></div>
        <button className="add-tax-bracket" type="button" onClick={()=>setForm({...form,brackets:[...form.brackets,{lowerBound:'',upperBound:'',baseTax:'0',ratePercent:'0'}]})}>+ Add bracket</button>
        <div className="tax-actions"><div className="tax-preview"><label><span>Test compensation (PHP)</span><input type="number" min="0" value={income} onChange={e=>setIncome(e.target.value)}/></label><button type="button" onClick={calculate}>Preview tax</button>{preview&&<span>Taxable: <b>₱{Number(preview.taxableIncome).toLocaleString()}</b> · Withholding: <b>₱{Number(preview.tax).toLocaleString()}</b></span>}</div><button className="save-tax" disabled={saving||!(form.id?permission?.update:permission?.create)}>{saving?'Saving…':'Save tax table'}</button></div>
      </form>
    </div>
  </section>;
}
