...
payrollRouter.put('/employees/:employeeId', ...requirePermission('payroll_setup','update'), async (request,response,next) => {
  ...
  try {
    ...
    if(components.length>200||components.some((item)=>!['earning','deduction','contribution'].includes(item.type)||!String(item.name||'').trim()||String(item.name).trim().length>100||!calculations.has(item.calculation)||!validMoney(Number(item.amount))||(item.calculation==='percentage'&&Number(item.amount)>100)||!benefitCategories.has(String(item.benefitCategory||''))||(item.type!=='earning'&&item.benefitCategory)))return response.status(400).json({error:'Enter valid earnings, deductions, contributions, and statutory benefit categories.'});
