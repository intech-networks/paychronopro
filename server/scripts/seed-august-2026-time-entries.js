...
const sampleAgentIds = ['paytimepro-dummy-data', 'paytimepro-sample-august-2026'];
...
try {
  ...
  const inserted = await client.query(`
    WITH active_employees AS (
      ...
      CROSS JOIN GENERATE_SERIES(
        DATE '2026-08-01', DATE '2026-08-31', INTERVAL '1 day'
      ...
      SELECT COALESCE(JSONB_AGG(JSONB_BUILD_OBJECT(
        ...
        'source', 'sample-august-2026'
      ) ORDER BY local_timestamp, employee_number, sequence), '[]'::jsonb) AS records,
      COUNT(*)::integer AS punch_count,
      COUNT(DISTINCT employee_number)::integer AS employee_count,
      COUNT(DISTINCT (employee_number, work_date))::integer AS workday_count
      FROM local_punches
    ), device_users AS (
      SELECT COALESCE(JSONB_AGG(JSONB_BUILD_OBJECT(
        'userId', employee_number
    ...
    SELECT 'paytimepro-sample-august-2026',
           TIMESTAMPTZ '2026-08-31 18:00:00+08',
           '{"name":"August 2026 Sample Attendance","serialNumber":"DEMO-AUG-2026"}'::jsonb,
  ...
  const coverage = await client.query(`
    ...
    JOIN employee_profiles employee
      ON attendance.entry->>'userId' = employee.employee_number
    WHERE backup.agent_id = 'paytimepro-sample-august-2026'
  `);
  ...
  console.log(JSON.stringify({
    ...
    replacedSampleBackups: removed.rowCount,
    ...coverage.rows[0]
  }, null, 2));
} catch (error) {
  await client.query('ROLLBACK');
  console.error(`August 2026 time-entry seed failed: ${error.message || error}`);
  process.exitCode = 1;
} finally {
  client.release();
  await pool.end();
